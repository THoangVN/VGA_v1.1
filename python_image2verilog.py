...
def get_color_bits(im, y, x):
    ...
    if(str(rx+gx+bx) == "000000000000000000000000"):
        return str("000000000000000000000000000000")
    return str(rx+"11"+gx+"11"+bx+"11")
    #return str(rx+gx+bx)

# write to file Verilog HDL
# takes name of file, image array,
# pixel coordinates of background color to mask as 0
def rom_30_bit(name, im, mask=False, rem_x=-1, rem_y=-1):
    ...
    for y in range(y_max):
        for x in range(x_max):
            ...
            f.write("\t\t" + str(row_width + col_width) + "'b" + case + ": color_data = " + str(30) + "'b")
            ...
            if(mask == False):
                ...
                f.write("000000000000000000000000000000;\n")
                
        f.write("\n")
        
    # write end of module
    f.write("\t\tdefault: color_data = 30'b000000000000000000000000000000;\n\tendcase\nendmodule")
...
def generate(name, rem_x=-1, rem_y=-1):
    ...
    im = imageio.v2.imread(name)			    # change2 to     
...
generate("eagle.bmp")     # <-- change your .bmp file name here
